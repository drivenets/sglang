...
class MooncakeTransferEngine:
    ...
    def batch_transfer_async(
        ...
        """Submit an asynchronous batch transfer. Returns a batch_id (>0) on
        successful submission, 0 on failure. Poll with
        ``get_batch_transfer_status`` to wait for completion. Used by the
        coalescing transfer worker to overlap RDMA across destination sessions.
        """
        try:
    ...
    def get_batch_transfer_status(self, batch_ids: List[int]) -> int:
        """Wait for async batch transfers to complete. Returns 0 on success,
        non-zero on failure."""
