...
class SWAKVPool(KVCache):
    ...
    def translate_loc_from_full_to_swa(self, kv_indices: torch.Tensor):
        ...
        # kv_indices may contain -1 sentinels from alloc_extend; the last slot of
        # full_to_swa_index_mapping is -1, so -1 → -1. Clamp unconditionally (idempotent
        # for in-range values); a device->host `.any()` check is incompatible with CUDA
        # graph capture, which is on the decode hot path.
        max_valid = self.full_to_swa_index_mapping.shape[0] - 1
