...
class ZmqEventPublisher(EventPublisher):
    ...
    def _publisher_thread(self) -> None:
        ...
        logger.info("ZMQ publisher thread started (endpoint=%s, topic=%r)", self._endpoint, self._topic_bytes)
        ...
        try:
            while self._running or self._event_queue.qsize() > 0:
                ...
                try:
                    ...
                    if event is None:
                        logger.info("ZMQ publisher thread received shutdown sentinel")
                ...
                try:
                    ...
                    self._send_count += 1
                    if self._send_count <= 5 or self._send_count % 100 == 0:
                        num_events = len(event.events) if hasattr(event, 'events') else '?'
                        logger.info("ZMQ publisher: sent batch seq=%d (total=%d, events=%s, payload_size=%d)",
                                    seq, self._send_count, num_events, len(payload))
            ...
            logger.exception("ZMQ publisher thread CRASHED: %s", e)
        finally:
            logger.info("ZMQ publisher thread exiting (sent %d batches total)", self._send_count)
