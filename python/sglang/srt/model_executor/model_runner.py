    ...
    def _forward_raw(
        ...
        if (
            ...
            if forward_batch.forward_mode.is_extend():
                ...
                # but only when the splitter could pick a viable split point.
                # split_idx is None when bs<2 or the split would produce an
                # empty micro-batch — in that case fall through to the
                # standard non-TBO forward path. (See PD-disagg warmup
                # crash on bs=1 EXTEND for why this gate is necessary.)
