    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        if self.block_quant:
            ...
            if not self.quant_config.is_checkpoint_fp8_serialized:
                # Check if we should use 2D block quantization for blockscale kernel
                use_2d_block_quant = (_use_aiter and 
                                     get_bool_env_var("SGLANG_FP8_USE_BLOCKSCALE"))
                
                if use_2d_block_quant:
                    # Use 2D block quantization (128x128) for AITER blockscale kernel
                    ...
                    # Don't transpose for blockscale kernel - it expects (N, K) format
                    layer.weight = Parameter(qweight, requires_grad=False)
                    layer.weight_scale = Parameter(weight_scale, requires_grad=False)
                    
                elif self.cutlass_fp8_supported or self.use_marlin or _use_aiter:
                    # apply per-channel quantization default as
                    # cutlass sgl-kernel, marlin, and AITER bpreshuffle support per-channel scale
...
class Fp8MoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        if not self.quant_config.is_checkpoint_fp8_serialized:
            ...
            # Use standard per-expert quantization (not per-channel)
            # Per-channel quantization for MoE doesn't seem to work correctly
            if False:  # Disabled per-channel MoE quantization
                pass
            else:
    ...
    def process_weights_hip_scale_padding(self, layer: Module):
        ...
        if _use_aiter:
            ...
            # The kernel expects: output * weight_scale1 * weight_scale
            # weight_scale1 stores relative per-channel factors
            # This multiplication converts them to absolute scales
